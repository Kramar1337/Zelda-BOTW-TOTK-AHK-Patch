...
version := "v0.4.1.4 mod by Kramar1337"
...
IniRead, key_overlay, ksettings.ini, KSettings, key_overlay
IniRead, key_map, ksettings.ini, KSettings, key_map
IniRead, key_reload, ksettings.ini, KSettings, key_reload
IniRead, key_amiibo1, ksettings.ini, KSettings, key_amiibo1
IniRead, key_amiibo2, ksettings.ini, KSettings, key_amiibo2
IniRead, key_amiibo3, ksettings.ini, KSettings, key_amiibo3
IniRead, key_lockq, ksettings.ini, KSettings, key_lockq
IniRead, key_lockR, ksettings.ini, KSettings, key_lockR
...
Hotkey, %key_reload%, Metkakey_reload, on
Hotkey, %key_amiibo1%, Metkakey_amiibo1, on
Hotkey, %key_amiibo2%, Metkakey_amiibo2, on
Hotkey, %key_amiibo3%, Metkakey_amiibo3, on
Hotkey, *~$%key_lockq%, Metkakey_lockq, on
Hotkey, *~$%key_lockR%, Metkakey_lockR, on


Hotkey,LButton,, off
Hotkey,Tab,, off

gameexe1337 = ahk_exe yuzu.exe

FileRead, GroupNameMap1337Var, GroupNameMap1337.txt
...
Random, RandomVar , 1, 2
if RandomVar = 1
BotwOverlayRandom:="ZeldaBotwOverlay1.png"
if RandomVar = 2
BotwOverlayRandom:="ZeldaBotwOverlay2.png"
Gui, 99: +AlwaysOnTop +ToolWindow -Caption +LastFound -DPIScale
Gui, 99: Color, 0x000000
Gui, 99: Add, Picture, w%A_ScreenWidth% h%A_ScreenHeight% x0 y0, %BotwOverlayRandom%
Gui, 99: Show, Hide w%A_ScreenWidth% h%A_ScreenHeight% x0 y0, %BotwOverlayRandom%
Gui, 99: Cancel
...
overwriteWheelUp:
	...
	SetStick(0,0)
	SetTimer, ReleaseDPad, -650 ; vstick.SetBtn(0,16)  ;выбор оружия
Return
overwriteWheelDown:
	SetStick(0,0)
	IF (!alreadyDown){
...
SubmitAll:
	...
	;==========================================Ща починим раскладку
	opupKey := GetHotkeySC(opupKey)
	opleftKey := GetHotkeySC(opleftKey)
	opdownKey := GetHotkeySC(opdownKey)
	oprightKey := GetHotkeySC(oprightKey)
	opwalkToggleKey := GetHotkeySC(opwalkToggleKey)
	opincreaseWalkKey := GetHotkeySC(opincreaseWalkKey)
	opdecreaseWalkKey := GetHotkeySC(opdecreaseWalkKey)
	opgyroToggleKey := GetHotkeySC(opgyroToggleKey)
	;==========================================Но не все
...
;===========================================================================20 Heart Wolf Link.bin===волк 20хп
Metkakey_amiibo1:
IfWinNotExist, %gameexe1337%
Return
if !toggle
{
...
sleep 200
ControlSend,, {F2},%gameexe1337%
sleep 1000
SendInput, C:\Amiibo1337\20 Heart Wolf Link.bin
sleep 500
SendInput {Enter}
sleep 200
WinActivate %gameexe1337%
...
ControlSend,, {F2},%gameexe1337%
sleep 1000
SendInput, C:\Amiibo1337\June TP Link.bin
sleep 500
SendInput {Enter}
sleep 200
WinActivate %gameexe1337%
...
;===========================================================================BOTW Guardian.bin===детали стражей и меч\щит
Metkakey_amiibo3:
IfWinNotExist, %gameexe1337%
Return
if !toggle
...
ControlSend,, {F2},%gameexe1337%
sleep 1000
SendInput, C:\Amiibo1337\BOTW Guardian.bin
sleep 500
SendInput {Enter}
sleep 200
WinActivate %gameexe1337%
...
if (toggle1)
{
	...
	{
		Gosub, controllerSwitch
		run_param="https://www.zeldadungeon.net/tears-of-the-kingdom-interactive-map"
...
if !toggle
{
	vstick.SetBtn(1,5) ;действие камня шика
	sleep 30
	KeyWait, %key_lockq%, T15
	vstick.SetBtn(0,5) ;действие камня шика
}
Sleep 1
Return

;=========================Прокручивание комбокинез
Metkakey_lockR:
if !toggle
{
	vstick.SetBtn(1,6) ;бросить предмет
	sleep 30
	KeyWait, %key_lockR%, T15
	vstick.SetBtn(0,6) ;действие камня шика
}
Sleep 1
Return

;vstick.SetBtn(0,2) ;это спринт
;vstick.SetBtn(0,3) ;прыжок
;vstick.SetBtn(0,4) ;атака 1
;vstick.SetBtn(0,1) ;использовать
;vstick.SetBtn(0,5) ;действие камн шика
;vstick.SetBtn(0,6) ;бросить предмет
;vstick.SetBtn(0,7) ;блок щитом
;vstick.SetBtn(0,8) ;выстрел с лука
;vstick.SetBtn(0,9) ;инвентарь
;vstick.SetBtn(0,10) ;карта атлас
;vstick.SetBtn(0,11) ;присесть
;vstick.SetBtn(0,12) ;бинокль
;vstick.SetBtn(0,13) ;выбрать руну шика
;vstick.SetBtn(0,14) ;свист
;vstick.SetBtn(0,15) ;выбор щит
;vstick.SetBtn(0,16) ;выбор оружие
;vstick.SetBtn(0,17) ;хз что это,, дунуть в микрофон... что?
;vstick.SetBtn(0,18) ;экран блокировки
;vstick.SetBtn(0,19) ;хз что это, конец вроде уже на 18 кнопке
;SetStick(-1,1) ;крутнуть влево
;SetStick(-1,0) ;отменить кручение влево
